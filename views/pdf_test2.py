...
f = open('C:/Temp/excelexport.txt', 'r')
...
def exportexcel():
    ...
    excelwriter = pd.ExcelWriter(r"C:/Temp/excelexport.xlsx")
...
def exporttxt():
    txtfile = open("C:/Temp/excelexport.xlsx",'w') # (w = write)
...
def exportpdf():
    my_file1 = Path("C:/Temp/excelexport.txt")
    ...
    txtfile = open("C:/Temp/excelexport.txt", 'w') # w= write
    txtfile.write(str(df))
    txtfile.close()
    ##Opens the txt file and reads its contents to a variable and then  populates to a PDF file
    txtfile = open("C:/Temp/excelexport.txt",'r')
    for x in txtfile:
        pdf.cell(200,10,txt=x,ln=1,align='c')
    pdf.output("C:/Temp/pdfexport.pdf")
